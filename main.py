...
def choose_upl():
    global width_window, height_window
    clock = pg.time.Clock()
    f2 = pg.font.SysFont('serif', 25)
    text = "Enter the number of maze (an integer from 1 to " + str(kl - 1) + "):"
    point_numb = f2.render(text, True, colors.WHITE)
    place_numb = point_numb.get_rect(center=(width_window * 2 / 5, height_window / 8))
    ...
    while not done:
        ...
        screen.fill((30, 30, 30))
        screen.blit(point_numb, place_numb)
        for box in input_boxes:
            box.draw(screen)
